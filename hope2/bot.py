...
class Pawn:
	...
	def check_safe_capture(self, dir):
		...
		if self.local(0, dir * 2) != team and self.local(-1, 0) != team and self.local(-1, -1) == team and self.local(1,
		                                                                                                              -1) == team:
			log("Unsafe capture")
			return False
		return True
...
class Overlord:
	...
	def safe_spawn(self, i):
		if not inbounds(self.index, i):
			return False
		if self.get_pos(self.index + self.forward, i - 1) == opp_team or self.get_pos(self.index + self.forward,
		                                                                              i + 1) == opp_team:
			return False
